...
def prepareJson(file_path):
    file_list = os.listdir(file_path)
    data = {}
    for ith, name in enumerate(file_list):
        print(ith, '/', len(file_list))
        coor = name.split('-')[2]
        xy = coor.split('_')
        x1,y1 = list(map(int,(xy[0].split('&'))))
        x2,y2 = list(map(int,(xy[1].split('&'))))
        labels = 'palte'
        coor = [x1,y1,x2,y2,labels]
        data[name] = [coor]
    return data

def main(srcDir, dstDir,srcimg,dstimg):
    objs = prepareJson(dstimg)
    trainval = []
    if len(objs):
        for di,(name, ob) in enumerate(objs.items()):
            print(di,'/',len(objs))
            name_tou = name.split('.')[0]
            tmp = PascalVocWriter(dstDir, name_tou, (720, 1160, 3), srcDir)
            for obj in ob:
                tmp.addBndBox(obj[0], obj[1], obj[2], obj[3], obj[4])
            tmp.save()
            trainval.append(name_tou)
    return trainval

if __name__ == '__main__':
    dstann = 'data/VOC2007/Annotations'
    dstimg = 'data/VOC2007/JPEGImages'
    dsttxt = 'data/VOC2007/ImageSets/Main'
    os.makedirs(dstann,exist_ok=True)
    os.makedirs(dstimg,exist_ok=True)
    os.makedirs(dsttxt,exist_ok=True)

    trainval = main(dstann,dstann, dstimg,dstimg)
    trainval_path = os.path.join(dsttxt,'trainval.txt')
    with open(trainval_path,'w') as f:
        for path in trainval:
            f.writelines(path+'\n')

    trainval_path = os.path.join(dsttxt,'test.txt')
    with open(trainval_path,'w') as f:
        for path in trainval[20000:]:
            f.writelines(path+'\n')
